...
Afin de loguer les différentes informations, il est important d'avoir la date des mesures.
...
On veut faire tourner le script automatiquement chaque minute de façon à avoir 1h de logs.<br>
...
Ce script lit le fichier CSV précédemment généré, place ses valeurs dans différents tableaux et les place dans des endroits spécifiques d'un template HTML.
...
Le script est construit de la façon qui suit:
 ...
 - Les valeurs de chaque capteur sont recopiées dans un tableau associé (processeur, mémoire et disque) en sautant une case quand la différence de temps entre les mesures est supérieure à une minute
...
!!! danger "Attention aux yeux"
    ...
    Le script a été écrit dans la précipitation, et en partie copié sur le premier script de Tim (voir sources).
 
    Nous vous prions de ne pas lire ce qui suit sans protection oculaire.
...
## Sources
 ...
 - [fedingo.com](https://fedingo.com/how-to-run-python-script-in-apache-web-server/)
 - [apache.org](https://httpd.apache.org/docs/current/fr/howto/cgi.html)
